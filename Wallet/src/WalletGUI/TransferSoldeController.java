...
import static WalletGUI.ForgotPasswordController.code;
...
public class TransferSoldeController implements Initializable {
    ...
    private void env(ActionEvent event) throws SQLException {
        ...
        if(sww.Checksolde(sww.getOneByUserId(sessionManager.getCurrentUser().getId()).getId(), Integer.parseInt(tfmon.getText())))
        {
        Wallet w = new Wallet();
        ...
        sww.AddSolde(w,Integer.parseInt(tfmon.getText()) );
        Utilisateur u ;
        u=w.getIduser();
        Transaction t ;
        Notification n;
